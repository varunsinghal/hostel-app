...
$sqltoperform = 'SELECT * FROM student NATURAL JOIN distance_from_home NATURAL JOIN docu_submission NATURAL JOIN academic NATURAL JOIN available_room WHERE student.student_id = distance_from_home.student_id AND docu_submission.document=1 AND student.gender = "MALE" AND student.student_id = academic.student_id AND student.student_id = available_room.student_id ORDER BY distance_from_home.distance DESC';
