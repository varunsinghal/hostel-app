...
<script src="side_functions.js"></script>


<h1 style="color: #CCC;">Apply Filter V.2</h1>
<br />
...
<tr>
	...
	<select name="category" id="form-category">
		...
		<?php
			$query = "SELECT DISTINCT category_code FROM student";
	...
	<select name="yr_admission" id="form-yr-admn">
		<option value="none">Show all</option>
		<?php
			$query = "SELECT DISTINCT year_of_admn FROM academic ORDER BY year_of_admn ASC";
			$result = mysql_query($query);
			while($data = mysql_fetch_array($result))
			{
	...
	<select name="course" id="form-course">
		<option value="none">Show all</option>
		<?php
			$query = "SELECT DISTINCT course FROM academic";
			$result = mysql_query($query);
			while($data = mysql_fetch_array($result))
			{
...
{
	$heading = '';
	$natural_joins = 'NATURAL JOIN distance_from_home ';
	...
	{
		$similarquery1 = "SELECT name,father_name FROM student WHERE student_id='$id' LIMIT 1";
		$resultsim1 = mysql_query($similarquery1);
		$simdata1 = mysql_fetch_array($resultsim1);
		$name = $simdata1['name'].'<br><br>';
		$father_name = $simdata1['father_name'];
		$similarquery2 = "SELECT count(*) FROM student WHERE name='$name' AND father_name='$father_name'";
	...
	function display_after_filtering($heading, $natural_joins, $conditions, $order)
	{
		...
		if(mysql_num_rows($result))
		{
			...
			echo '<table width=100% id="gTable"><tr><th>S.No</th><th>Form No.</th><th>Academics</th><th>Name</th><th>Type</th><th>Present Address</th><th>Distance</th><th>Category</th><th>Room No</th><th>Actions</th></tr>';
		}
		else {
			echo '<h2 style="color:#FF6633">No entries found!!</h2>';
		}		
		$i = 1; // $i is for serial number
	...
	tr id="'.$row['student_id'].'">
			...
			<td>'.$reallot.'&ensp;<br/>
			<img src="../photo/'.$reset['file'].'" height = "150">
			</td>
