...
<h1><span style="color:#FF3D3D;">Unverified Applications</span><span style="color: #CCC;"> - Apply Filter V.2</span></h1>
<br />
...
<tr>
	...
	<select name="category" id="form-category">
		...
		<?php
			$query = "SELECT student_id,category_code FROM student NATURAL JOIN docu_submission WHERE student.student_id = docu_submission.student_id AND docu_submission.document = 0 GROUP BY student.category_code";
	...
	<select name="yr_admission" id="form-yr-admn">
		<option value="none">Show all</option>
		<?php
			$query = "SELECT student_id,year_of_admn FROM academic NATURAL JOIN docu_submission WHERE academic.student_id = docu_submission.student_id AND docu_submission.document = 0 GROUP BY academic.year_of_admn";
			$result = mysql_query($query);
			while($data = mysql_fetch_array($result))
			{
	...
	<select name="course" id="form-course">
		<option value="none">Show all</option>
		<?php
			$query = "SELECT student_id,course FROM academic NATURAL JOIN docu_submission WHERE academic.student_id = docu_submission.student_id AND docu_submission.document = 0 GROUP BY academic.course";
			$result = mysql_query($query);
			while($data = mysql_fetch_array($result))
			{
...
if(isset($_POST['submit']))
{
	...
	$conditions = 'student.student_id = distance_from_home.student_id AND docu_submission.document=0 ';
	$order = '';
