...
<h1 style="color: #CCC;">Hostel Roll Number Manager</h1>
<br />
...
if(isset($_POST['submit']))
{
	...
	{
		echo "<center>Roll Number Format :<br /><br /><input type='text' value='2K10' size='5' id='rollformatyear' onkeyup='changeRollNoSubmit();'>/<input type='text' id='rollformattext' onkeyup='changeRollNoSubmit();'>/serial-number <a href='javascript:void(0);' onClick='changeRollNoSubmit();'>Submit</a></center><br />";
		echo "<center><input type='button' value='APPLY' style='font-size:14px;font-weight:bold;width:100px;background-color:#33FF66;border-style:solid;border-color:#000000;' onClick='submitallhrollno();'></center><br />";
		...
		if(mysql_num_rows($result))
		{
			...
			echo '<table width=100% id="gTable"><tr><th>S.No | F. No</th><th>Academics</th><th>Name</th><th>Present Address</th><th>Distance</th><th>Category</th><th>Room No</th><th width=180px>Hostel Roll No</th></tr>';
		}
		else {
		...
		while($row = mysql_fetch_array($result)){
			...
			echo '</td>
			<td><input type=text id="rollinput'.$row['student_id'].'" value="'.$row['recipt'].'"><br /><a href="view.php?page='. $row['student_id'].'" target="_blank" onclick="return popitup(&#39;view.php?page='. $row['student_id'].'&#39;)">View Full Details</a><br /><a href="edit_student_info.php?id='.$row['student_id'].'">Edit Details</a> | <a href="javascript:void(0);" onClick="addRemark('.$row['student_id'].')">Add Remark</a></td>
			</tr>
			<tr><td colspan=8 id="'.$row['student_id'].'a">';
			...
			echo '</td></tr>
			<tr><td colspan=8 id="'.$row['student_id'].'b"></td></tr>';
			$i++;
		}
