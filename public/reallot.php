...
$query = "SELECT * FROM control_variables WHERE control = 'reallotment' LIMIT 1";
...
function hostel_roll_course(course_name){
  if(course_name=='BTECH')
  document.getElementById('f46').innerHTML="HO/";
  else
  document.getElementById('f46').innerHTML=course_name+"/HO/";
}
function hostel_roll_year(year_k){
  if(year_k!=""){
  var year_k = year_k.split("");
  year_k[1]='K';
  year_k=year_k[0]+year_k[1]+year_k[2]+year_k[3];
  document.getElementById('f45').innerHTML = year_k+"/";
  }
}
function hostel_roll_gender(gender_value){
  if(gender_value=='FEMALE')
  document.getElementById('f47').innerHTML = "G/";
  if(gender_value=='MALE')
  document.getElementById('f47').innerHTML = "";
}
function univer_roll(univer_roll_sem){
  if(univer_roll_sem<=3)
  document.getElementById('f50').innerHTML = "University Registration No.";
  else
  document.getElementById('f50').innerHTML = "University Roll No.";
}
function get_rooms(hos) {
	document.getElementById('f52').innerHTML = "<center><img src='images/ajax-loader_b.gif' height=24 /></center>'";
	...
	xmlhttp.onreadystatechange = function () {
		if (xmlhttp.readyState == 4 && xmlhttp.status == 200) {
			document.getElementById('f52').innerHTML = xmlhttp.responseText;
		}
	}
	xmlhttp.open("GET", "layouts/get_rooms.php?hos=" + hos, true);
	xmlhttp.send();
}
...
<form method="post" action="display_reallot.php" enctype="multipart/form-data" onsubmit="return confirm('Submit the form?');">
...
<tr>
    <td>
        Course</td><td><?php include "layouts/course.txt"; ?>
    </td>
</tr>
...
<tr>
    ...
    <td>
        <input type="text" name="sem" size="50" id="f1" onchange="univer_roll(this.value)"/>
        <script type="text/javascript">
...
<tr>
        ...
        <td>
            <input type="text" name="year_of_admn" id="f2" size="50" onchange="hostel_roll_year(this.value)" />
            <script type="text/javascript">
...
<tr>
    <td id="f50">University Roll No.</td>
        <td>
...
<tr><th>Basic Details</th></tr>
      ...
      <tr>
        <td>Gender</td>
        <td>
          <input type="radio" name="gender" value="MALE" checked="checked" onclick="hostel_roll_gender(this.value)" id="f48"/><label for="f48">Male</label><input type="radio" name="gender" value="FEMALE" onclick="hostel_roll_gender(this.value)" id="f49"/><label for="f49">Female</label>
        </td>
      </tr>
      <tr>
      ...
      <tr>
        <td>Hostel Roll Number</td>
        <td>
            <span id="f45"></span><span id="f46">HO/</span><span id="f47"></span><input type="text" name="recipt" id="f7" size="15" />
            <script type="text/javascript">
		var f7 = new LiveValidation('f7', { validMessage: "Correct!"});
                f7.add( Validate.Presence );
                f7.add( Validate.Numericality );
                f7.add( Validate.Length, { is: 3 } );
	    </script>
        </td>
      </tr>
      <tr>
        <td>Admission Category</td><td><?php include "layouts".DS."category.txt"; ?></td>
      </tr>
      <tr>
      ...
      <tr>
        <td>Last Year Room no and Hostel</td>
        <td>
            <?php include "layouts".DS."lastroom.txt"; ?>
        </td>
      </tr>
      <tr>
...
<tr><th>Enter the Captcha</th></tr>
      ...
      <tr>
        <td></td><td><input type="submit" value="Continue" id="continue_btn" disabled="disabled" name="submit" size="50" style="visibility: hidden;" /><noscript><font size="+6" color="red">Enable Javascript to see complete form</font></noscript></td>
      </tr>
    </table>
</form>
