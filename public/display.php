   ...
   exit;
	...
	if(!empty($_FILES)){
		...
		if($ext!='jpg' && $ext!='jpeg' && $ext!='gif' && $ext!='png' && $ext!='JPG' && $ext!='JPEG' && $ext!='GIF' && $ext!='PNG'){
			redirect_to('register.php?error=fileformat&code='.$ext);
			exit;
		}
		else if($file_size>2097152)
		{
			redirect_to('register.php?error=filesize&code='.$filesize);
			exit;
		}
...
if(isset($_POST['submit'])){
  ...
  //make last year record
  if($_POST['course'] == 'BTECH' && $_POST['sem'] == 1 ){
  $_POST['lr1'] = 'JEE MAIN Rank: '.$_POST['lr1'];
  $_POST['lr2'] = 'JEE MAIN Score: '.$_POST['lr2'];
  }
  else if($_POST['course'] == 'MTECH'){
  $_POST['lr1'] = 'GATE Score: '.$_POST['lr1'];
  $_POST['lr2'] = 'GATE Rank: '.$_POST['lr2'];
  }
  else if($_POST['course'] == 'MBA'){
  $_POST['lr1'] = 'CAT Score: '.$_POST['lr1'];
  $_POST['lr2'] = 'CAT Percentile: '.$_POST['lr2'];
  }
  else{
  $_POST['lr1'] = 'Last Sem: '.$_POST['lr1'];
  $_POST['lr2'] = '2nd Last Sem: '.$_POST['lr2'];
  }
  $student = new Student(); 
  ...
  $student->reallot               = 0;
  $student->last_sr1	          = $_POST['lr1'];
  $student->last_sr2              = $_POST['lr2'];
  //academic_details
  ...
  '{lg_office_phone}'=>$_POST['lg_office_phone']);
	$html = file_get_contents ('layouts/register.inc.php');
	foreach($attribute as $key => $value){
  		$html = str_replace ("{$key}", $value, $html);
	}
	$filename = "html".DS.time().$student->student_id.".html";
	file_put_contents("mpdf".DS.$filename, $html);
	redirect_to('get_register.php?input_file=mpdf'.DS.$filename);
    	
    $message = "";
  } else {
    $message = "Form submission Failed. <a href='register.php'>Click Here</a> to refill the form.";
  }
}
